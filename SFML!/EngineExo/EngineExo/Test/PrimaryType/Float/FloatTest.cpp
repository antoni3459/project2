...
void Test::FloatTest::Test()
{
	...
	checkLow((_d == 25.0f), "d is not equals to a + b = 25.0f")
		_d = _b - _a;
	checkLow((_d == 5.0f), "d is not equals to b - a = 5.0f")
		_d = _a * _b;
	checkLow((_d == 150.0f), "d is not equals to a * b = 150.0f")
		_d = _a / 2.0f;
	checkLow((_d == 5.0f), "d is not equals to a / 2 = 5.0f")
		_d += 250;
	checkLow((_d == 255.0f), "d is not equals to d += 250 = 225.0f")
		_d -= 30.0f;
	checkLow((_d == 225.0f), "d is not equals to d -= 30 = 225.0f")
		_d *= 2;
	checkLow((_d == 450.0f), "d is not equals to _d *= 2 = 450.0f")
		_d /= 4;
	checkLow((_d == 112.5f), "d is not equals to d /= 4 = 112.5f")
	...
	checkLow((Engine::PrimaryType::Float::IsPositiveInfinity(_d)), "d is not equals to Nan")
		_d = Engine::PrimaryType::Float::NegativeInfinity;
	checkLow((Engine::PrimaryType::Float::IsNegativeInfinity(_d)), "d is not equals to Nan")
